...
class page_reports_gst extends \xepan\commerce\page_reports_reportsidebar{
	...
	function init(){
		...
		$m->addExpression('customer_organization')->set(function($m,$q){
			$contact = $m->add('xepan\base\Model_Contact');
			$contact->addCondition('id',$m->getElement('customer_id'));
			return $q->expr('[0]',[$contact->fieldQuery('organization')]);
		});
